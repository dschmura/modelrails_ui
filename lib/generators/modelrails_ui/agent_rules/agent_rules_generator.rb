...
module ModelrailsUi
  module Generators
    ...
    class AgentRulesGenerator < Rails::Generators::Base
      ...
      AGENT_RULES_PATH = ".modelrails_ui/agent-rules.md"
      ...
      def write_agent_rules
        ...
        copy_file "agent-rules.md", AGENT_RULES_PATH, force: true
        # After the copy, never through it: the rules file contains a literal
        # `<%= ui :input, … %>` example, so running it through ERB as a `template`
        # would execute it. The overwrite above restores the unstamped template
        # first, so a re-run leaves exactly one stamp (#175).
        prepend_to_file AGENT_RULES_PATH, version_stamp
      end
      ...
      # The host copy is deliberately git-excluded, which removes every ordinary
      # staleness signal — no PR diff, no sync conflict, no `git status` entry. This
      # file is instruction text an agent reads and obeys, so a stale copy does not
      # sit inertly, it actively directs work. The stamp names the producing version
      # and how to refresh it.
      def version_stamp
        "<!-- generated by modelrails_ui #{ModelrailsUi::VERSION} — " \
          "re-run `rails g modelrails_ui:agent_rules` after a gem bump -->\n\n"
      end
