...
export default class extends Controller {
  ...
  #buildToast({ message = "", title = "", severity = "default", variant, duration = 4000 }) {
    ...
    // Tints, never fades — an opacity fade reads as DISABLED and drags the icon
    // toward the toast's own surface. Keyed by severity, because the ground
    // differs per tone. Mirrors toaster_component.rb's SEVERITIES[:close_hover];
    // the two copies render the same control and have drifted before (#256).
    const closeHoverCls = {
    ...
    div.innerHTML = `
      ...
      <button type="button" aria-label="Dismiss"
        ...
        class="ml-auto -mr-2 -mt-1.5 shrink-0 inline-flex size-11 items-center justify-center
               rounded-md text-current focus-ring transition ${closeHoverCls}">
