...
export default class extends Controller {
  ...
  async copy() {
    ...
    try {
      ...
      // The empty-value/no-clipboard branches above throw with no prior await, so without
      // this yield the clear from #clearRegions() and this failure write would land in the
      // same mutation batch — the intermediate "" would never be observable, and a second
      // failure would read as a same-text rewrite. Yielding once makes the failure
      // announcement its own mutation batch, same as the success path gets for free from
      // awaiting writeText.
