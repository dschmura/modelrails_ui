...
class TestAgentRulesGenerator < Minitest::Test
  ...
  # The host copy is git-excluded by design, so none of the usual staleness signals
  # exist — no PR diff, no sync conflict, no `git status` entry. A stamp is the only
  # thing that lets a human or an agent opening the file see it has fallen behind
  # the installed gem (#175). It matters more than ordinary doc drift because this
  # file is instruction text an agent reads and obeys.
  def test_generated_rules_are_stamped_with_the_producing_version
    Dir.mktmpdir do |dest|
      run_agent_rules(dest)
      rules = File.read(File.join(dest, ".modelrails_ui/agent-rules.md"))

      assert_includes rules, "generated by modelrails_ui #{ModelrailsUi::VERSION}"
      assert_includes rules, "modelrails_ui:agent_rules",
        "the stamp should say how to refresh the file, not just that it is old"
    end
  end

  # The stamp is prepended after the copy rather than interpolated, because the
  # template legitimately contains a literal `<%= ui :input, ... %>` in a code
  # example — running it through ERB would execute it. This asserts that example
  # still arrives verbatim.
  def test_the_erb_example_in_the_template_is_not_evaluated
    Dir.mktmpdir do |dest|
      run_agent_rules(dest)
      rules = File.read(File.join(dest, ".modelrails_ui/agent-rules.md"))

      assert_includes rules, "<%= ui :input, **f.input_attrs %>"
    end
  end

  # Re-running must not stack stamps on top of each other.
  def test_rerunning_leaves_exactly_one_stamp
    Dir.mktmpdir do |dest|
      run_agent_rules(dest)
      run_agent_rules(dest)
      rules = File.read(File.join(dest, ".modelrails_ui/agent-rules.md"))

      assert_equal 1, rules.scan("generated by modelrails_ui").length
    end
  end
