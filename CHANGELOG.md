...
read the notes before bumping. One makes a control taller to meet the AAA target
floor; the other changes generated ids for field names that contain brackets.
Both were found by adopting these components in the reference app rather than by
review, and one of them retires a divergence that app had been carrying locally.
...
the three removes a reason for calling code to compensate: an `mb-*` workaround
at every call site that slots an empty-state icon, a panel width the caller had
to keep an eye on, and a per-option description that previously meant
hand-rolling radios and losing the group's accessibility contract to get.
...
- **`empty_state`** — the placeholder shown where a list, table or panel would be if it had anything in it: an optional icon slot, a title, an optional supporting line, and an optional action slot. Added on evidence rather than speculation: the reference app had hand-rolled it seven times, in three shapes, which is where the three variants come from — `dashed` (the default well), `outlined` (a filled card), `plain` (no chrome, for a no-results block inside an existing panel). The title renders as a `<p>`, never a heading: an empty state can appear anywhere on a page, so emitting an `<h2>`/`<h3>` would inject a heading at an arbitrary level and break the outline for anyone navigating by headings. Closes #135.
...
- **The card family agrees on one surface — `bg-surface-raised`.** `list_group` and `table` moved off `bg-surface`, and `data_table`'s wrapper gained a surface where it previously had none, so a card, a list and a table side by side now read as the same kind of container. `bg-surface` is the **page**: a container painted with it is the same colour as the ground beneath it, and whether that looked deliberate depended entirely on the host's page background. **This changes the rendered appearance of `list_group`, `table` and `data_table` in every host that has vendored them** — and a host whose `<body>` is `bg-surface-raised` should move it to `bg-surface`, or every card-shaped container will flatten into the page. The rule is stated in `docs/design-tokens.md` and enforced by `test/test_container_surface.rb`, which scores every bordered container so a fourth answer cannot appear quietly. Closes #210.
...
- `label`: the required mark hugs its caption. It rendered as a sibling flex item under the label's `gap-2`, so the asterisk sat a full 8px from the text — "Email address  *" reads as a typo rather than a marker. Caption and mark are now one baseline-aligned text run; an unrequired label's DOM is unchanged. Closes #164.
- `modelrails_ui:agent_rules` stamps the generated file with the producing gem version and how to refresh it. The host copy is git-excluded by design, so nothing else could reveal it had fallen behind — no PR diff, no sync conflict, no `git status` entry — and it is instruction text an agent reads and obeys, so a stale copy actively directs work. Closes #175.
- `list_group_item`: `active: true` without `href:` now raises in development and test, and falls back to `default` in production. It marked a row as the current page while leaving it a plain, non-focusable `<li>` wearing the solid `bg-interactive` fill — current to the eye, and silent to assistive technology, since `static_row` rightly declines `aria-current`. Same fail-loud-then-degrade posture as the existing unknown-variant guard. The basic usage example in `docs/components/list_group.md` demonstrated exactly this combination and no longer does. Closes #196.
- `list_group`: a static row no longer highlights on hover. `hover:bg-surface-sunken` lived on the `default` and `muted` colour variants, so a plain non-focusable `<li>` lit up full-width and promised a click target that does not exist — sharpest in the common shape where the row is static and only a small child is actionable. Hover now lives in a variant-keyed map applied only on the link branch. The `active` cell is deliberately empty: an active row already carries the solid `bg-interactive` fill, and hovering it to `bg-surface-sunken` while it keeps `text-text-on-interactive` would be white text on a near-white surface in the light theme. Closes #191 and #197.
...
- `form_draft`: the restore announcement says what actually happened instead of always reporting a count. `serializeForm` uses `FormData`, which includes hidden inputs, while `recover()` refuses to write them back — so a draft key whose only fields are hidden (a rich-text editor's backing input) holds content that can never land, and the old message told a screen-reader user the draft came back whole. Now: nothing restored, a partial restore naming that some content could not be written back, or the count, with a singular form. A Rails checkbox hidden-`0` pair has a visible sibling and is correctly not treated as a partial restore. Closes #220.
