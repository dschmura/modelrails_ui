...
Render-test and app-adoption are now computed by `rake modelrails_ui:adoption`; this
ledger is human judgment (Tier + Notes) only.
...
| chip_group | proven | Checkbox-backed multi-select chips (#169 option B). Real inputs, so the selection posts natively and no client state can desync |
...
| table | proven | Server-rendered counterpart to `data_table` (sorting/filtering/paging are the caller's). Required caption; toolbar/header/body/footer slots; `scroll: :horizontal` wraps ONLY the `<table>` so the toolbar and pager stay pinned at phone width. Ported from the reference app, in production shape across four call sites since 2026-09-18 (#201) |
...
**Hardening program COMPLETE** — all 88 components are `proven` (0a + app 0b/AAA), `hardened` (gem 0a; `wysiwyg`/`toaster` are superseded in this host and adopt no app 0b), or `gem-0a-proven / app-0b-pending` (form_draft, form_builder, error_summary). No `experimental` components remain. (88 counts the `menubar_menu` sub-component row; the generator catalog itself is 87.)
